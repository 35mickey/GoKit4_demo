...
*/
	...
	status = i2c_write_reg_block(MPU6050_Addr,PWR_MGMT_1, 0x00);	//解除休眠状态
	if(status!=QAPI_OK)	{gizLog(LOG_ERROR,"[I2C ERROR] i2c_write_reg: status=%d\r\n", status);return;}

	status = i2c_write_reg_block(MPU6050_Addr,SMPLRT_DIV, 0x07);
	if(status!=QAPI_OK)	{gizLog(LOG_ERROR,"[I2C ERROR] i2c_write_reg: status=%d\r\n", status);return;}

	status = i2c_write_reg_block(MPU6050_Addr,CONFIG, 0x06);
	if(status!=QAPI_OK)	{gizLog(LOG_ERROR,"[I2C ERROR] i2c_write_reg: status=%d\r\n", status);return;}

	status = i2c_write_reg_block(MPU6050_Addr,GYRO_CONFIG, 0x18);
	if(status!=QAPI_OK)	{gizLog(LOG_ERROR,"[I2C ERROR] i2c_write_reg: status=%d\r\n", status);return;}

	status = i2c_write_reg_block(MPU6050_Addr,ACCEL_CONFIG, 0x01);
	if(status!=QAPI_OK)	{gizLog(LOG_ERROR,"[I2C ERROR] i2c_write_reg: status=%d\r\n", status);return;}
...
int32 read_MPU6050_gx(void)
{
	...
	status = i2c_read_reg_block(MPU6050_Addr,GYRO_XOUT_L,(uint8 *)&L_byte); 
	if(status!=QAPI_OK)	{gizLog(LOG_ERROR,"[I2C ERROR] i2c_read_reg: status=%d\r\n", status);return 0;}
	status = i2c_read_reg_block(MPU6050_Addr,GYRO_XOUT_H,(uint8 *)&H_byte);
	if(status!=QAPI_OK)	{gizLog(LOG_ERROR,"[I2C ERROR] i2c_read_reg: status=%d\r\n", status);return 0;}
