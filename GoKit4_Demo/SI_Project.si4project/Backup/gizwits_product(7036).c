...
static uint32_t _100ms_cnt = 0;/* 为了防止定时发送和回应发送冲突，设为全局，回应时清空 */
...
//	static uint8 key1_status_val = 0;/* 保存KEY1状态 */
	...
	/* 6s一次同步数据 */
	_100ms_cnt++;
	if(0 == _100ms_cnt%50)	/* 如果用SIG_IMM_REPORT，5s一次 */	
//	if(0 == _100ms_cnt%10)	/* 如果用SIG_UPGRADE_DATA，1s一次,实际只会在数据改变时更新 */
//	if(0 == _100ms_cnt%50)	/* 如果用SIG_UPGRADE_DATA,由于有alive time，所以也只能长一些 */
	{
		/************************************************/
...
//	adc_init();
	...
	oled_init();
	lcd_printf_display(0,0,"Hello! Qualcomm,");
	lcd_printf_display(2,0,"Gizwits &Quectel");
	lcd_printf_display(6,0,"    By Mickey   ");
