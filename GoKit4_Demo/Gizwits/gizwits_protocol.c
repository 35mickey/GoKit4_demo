...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
	...
	if(0x01 == issuedData->attrFlags.flagStop)
    {
        info->event[info->num] = EVENT_Stop;
        info->num++;
        dataPoints->valueStop = gizStandardDecompressionValue(Stop_BYTEOFFSET,Stop_BITOFFSET,Stop_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flagSpeed_value)
    {
        info->event[info->num] = EVENT_Speed_value;
        info->num++;
        dataPoints->valueSpeed_value = gizX2Y(Speed_value_RATIO,  Speed_value_ADDITION, issuedData->attrVals.valueSpeed_value); 
    }
        
    if(0x01 == issuedData->attrFlags.flagDirection_value)
    {
        info->event[info->num] = EVENT_Direction_value;
        info->num++;
        dataPoints->valueDirection_value = gizX2Y(Direction_value_RATIO,  Direction_value_ADDITION, issuedData->attrVals.valueDirection_value); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueStop != cur->valueStop)
    {
        gizLog(LOG_INFO,"valueStop Changed\n");
        ret = 1;
    }
    if(last->valueSpeed_value != cur->valueSpeed_value)
    {
        gizLog(LOG_INFO,"valueSpeed_value Changed\n");
        ret = 1;
    }
    if(last->valueDirection_value != cur->valueDirection_value)
    {
        gizLog(LOG_INFO,"valueDirection_value Changed\n");
        ret = 1;
    }
    if(last->valueGPS_Locked != cur->valueGPS_Locked)
    {
        gizLog(LOG_INFO,"valueGPS_Locked Changed\n");
        ret = 1;
    }
    if(last->valueKey1_value != cur->valueKey1_value)
    {
        gizLog(LOG_INFO,"valueKey1_value Changed\n");
        ret = 1;
    }

    if(last->valueAlive_seconds != cur->valueAlive_seconds)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO,"valueAlive_seconds Changed\n");
            ret = 1;
        }
    }
    if(0 != memcmp((uint8_t *)&last->valueGPS_long,(uint8_t *)&cur->valueGPS_long,sizeof(last->valueGPS_long)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO,"valueGPS_long Changed\n");
            ret = 1;
        }
    }
    if(0 != memcmp((uint8_t *)&last->valueGPS_lat,(uint8_t *)&cur->valueGPS_lat,sizeof(last->valueGPS_lat)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO,"valueGPS_lat Changed\n");
            ret = 1;
        }
    }
    if(0 != memcmp((uint8_t *)&last->valueGPS_Speed,(uint8_t *)&cur->valueGPS_Speed,sizeof(last->valueGPS_Speed)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO,"valueGPS_Speed Changed\n");
            ret = 1;
        }
    }
    if(0 != memcmp((uint8_t *)&last->valueGPS_acc,(uint8_t *)&cur->valueGPS_acc,sizeof(last->valueGPS_acc)))
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            gizLog(LOG_INFO,"valueGPS_acc Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Stop_BYTEOFFSET,Stop_BITOFFSET,Stop_LEN,(uint8_t *)devStatusPtr,dataPoints->valueStop);
    gizStandardCompressValue(GPS_Locked_BYTEOFFSET,GPS_Locked_BITOFFSET,GPS_Locked_LEN,(uint8_t *)devStatusPtr,dataPoints->valueGPS_Locked);
    gizStandardCompressValue(Key1_value_BYTEOFFSET,Key1_value_BITOFFSET,Key1_value_LEN,(uint8_t *)devStatusPtr,dataPoints->valueKey1_value);
    ...
    devStatusPtr->valueSpeed_value = gizY2X(Speed_value_RATIO,  Speed_value_ADDITION, dataPoints->valueSpeed_value); 
    devStatusPtr->valueDirection_value = gizY2X(Direction_value_RATIO,  Direction_value_ADDITION, dataPoints->valueDirection_value); 

    devStatusPtr->valueAlive_seconds = exchangeBytes(gizY2X(Alive_seconds_RATIO,  Alive_seconds_ADDITION, dataPoints->valueAlive_seconds)); 



    gizMemcpy((uint8_t *)devStatusPtr->valueGPS_long,(uint8_t *)&dataPoints->valueGPS_long,sizeof(dataPoints->valueGPS_long));
    gizMemcpy((uint8_t *)devStatusPtr->valueGPS_lat,(uint8_t *)&dataPoints->valueGPS_lat,sizeof(dataPoints->valueGPS_lat));
    gizMemcpy((uint8_t *)devStatusPtr->valueGPS_Speed,(uint8_t *)&dataPoints->valueGPS_Speed,sizeof(dataPoints->valueGPS_Speed));
    gizMemcpy((uint8_t *)devStatusPtr->valueGPS_acc,(uint8_t *)&dataPoints->valueGPS_acc,sizeof(dataPoints->valueGPS_acc));
    return 0;
}
...
//
	...
	/* 如果用SIG_IMM_REPORT，请取消下方更新数据点的注释 */
    gizMemcpy((uint8_t *)&gizwitsProtocol.gizLastDataPoint, (uint8_t *)dataPoint, sizeof(dataPoint_t));

	gizSendQueue(SIG_IMM_REPORT);	/* SIG_UPGRADE_DATA or SIG_IMM_REPORT */ 
