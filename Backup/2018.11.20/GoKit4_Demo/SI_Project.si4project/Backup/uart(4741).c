...
qapi_UART_Handle_t uart1_handle;
/* uart1 rx tx buffer */
char rx1_buff[UART_RX1_BUFF_SIZE]; /*!!! should keep this buffer as 4K Bytes */
char tx1_buff[UART_TX1_BUFF_SIZE];
#endif
	
#ifdef QT_UART_ENABLE_2ND
qapi_UART_Handle_t uart2_handle;
/* uart2 rx tx buffer */
...
qapi_UART_Handle_t uart3_handle;
/* uart3 rx tx buffer */
char rx3_buff[UART_RX3_BUFF_SIZE]; /*!!! should keep this buffer as 4K Bytes */
char tx3_buff[UART_TX3_BUFF_SIZE];
#endif
...
{
	UINT status;
	static qapi_UART_Open_Config_t uart1_cfg;
	static qapi_UART_Open_Config_t uart2_cfg;
	static qapi_UART_Open_Config_t uart3_cfg;

#ifdef QT_UART_ENABLE_1ST
	uart1_cfg.baud_Rate			= 115200;
	uart1_cfg.enable_Flow_Ctrl	= QAPI_FCTL_OFF_E;
	uart1_cfg.bits_Per_Char		= QAPI_UART_8_BITS_PER_CHAR_E;
	uart1_cfg.enable_Loopback 	= false;
	...
	status = qapi_UART_Receive(uart1_handle, (char *)rx1_buff, UART_RX1_BUFF_SIZE, (void*)NULL);
	...
	status = qapi_UART_Receive(uart3_handle, (char *)rx3_buff, UART_RX3_BUFF_SIZE, (void*)NULL);
	gizLog(LOG_INFO,"QT# qapi_UART_Receive [%d] status %d \n", QT_UART_PORT_03, status);
...
static void uart1_rx_cb(uint32_t num_bytes, void *cb_data)
{
	gizLog(LOG_INFO,"QT# qapi_UART_tx_cb [%d] num %d \n", QT_UART_PORT_01, num_bytes);
}
