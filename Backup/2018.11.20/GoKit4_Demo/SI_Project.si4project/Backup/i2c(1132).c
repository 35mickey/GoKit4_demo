...
//	qt_uart_dbg(uart_conf.hdlr,"i2c_read: slave_addr[0x%x], reg[0x%x]\r\n", device_Address, reg);
	...
	if(cnt < 10)
	{
		...
		res = qapi_I2CM_Transfer (i2c_handle, &i2c_config, &desc[0], 2, i2c_callback, &CB_Parameter, 100);
	}
	else
...
//	qt_uart_dbg(uart_conf.hdlr,"i2c_read: slave_addr[0x%x], reg[0x%x]\r\n", device_Address, reg);
	...
	{
		CB_Parameter = I2C_CB_BUSY;
		res = qapi_I2CM_Transfer (i2c_handle, &i2c_config, &desc[0], 1, i2c_callback, &CB_Parameter, 100);
	}
	else
		return QAPI_I2CM_ERR_TRANSFER_TIMEOUT;
