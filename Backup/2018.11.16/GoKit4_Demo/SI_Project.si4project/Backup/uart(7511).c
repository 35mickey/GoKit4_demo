...
/* uart1 rx tx buffer */
char rx1_buff[UART1_RX1_BUFF_SIZE]; /*!!! should keep this buffer as 4K Bytes */
char tx1_buff[UART1_TX1_BUFF_SIZE];
#endif
...
void user_uart_init(void)
{
	...
	status = qapi_UART_Ioctl(uart1_handle, QAPI_SET_BAUD_RATE_E, (void*)NULL);
	gizLog(LOG_INFO,"QT# qapi_UART_IO_Control [%d] status %d \n", QT_UART_PORT_01, status);
	...
	status = qapi_UART_Ioctl(uart3_handle, QAPI_SET_BAUD_RATE_E, (void*)NULL);
	gizLog(LOG_INFO,"QT# qapi_UART_IO_Control [%d] status %d \n", QT_UART_PORT_01, status);
