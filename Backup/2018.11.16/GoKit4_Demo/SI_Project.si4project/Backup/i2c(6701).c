...
qapi_I2CM_Config_t i2c_config;
unsigned char wbuff[128];

// Transfer completed
I2C_CB_Status_t CB_Parameter = I2C_CB_OK;
...
qapi_Status_t i2c_write (uint8 slave_Address, uint8 reg, uint8 *wbuf, uint8 len)
{
 ...
 //  static  uint8    buffer[4];
   ...
   wbuff[0] = reg; // first byte is register addr
   for (i = 0; i < len; ++i)
   {
       wbuff[i + 1] = wbuf[i];
   }
...
{
	qapi_Status_t res = QAPI_OK;
	static qapi_I2CM_Descriptor_t desc[2];
	static unsigned int transferred1 = 0;
	int cnt = 0;
	...
	// Configure the bus speed and slave address
	i2c_config.bus_Frequency_KHz = 50; 
	i2c_config.slave_Address     = device_Address;
	i2c_config.SMBUS_Mode        = 0;
	i2c_config.slave_Max_Clock_Stretch_Us = 100;
...
*/
	...
	//status = i2c_write(MPU6050_Addr,PWR_MGMT_1, &status,1);
	gizLog(LOG_INFO,"[I2C ERROR] PWR_MGMT_1 Finshed. \n");
