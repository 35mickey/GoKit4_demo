...
** <W> - Write Transfer
********************************************/
qapi_Status_t i2c_read_reg (uint8 device_Address, uint16 reg, uint8 *buff, I2C_REG_TYPE_t reg_type)
{
	qapi_Status_t res = QAPI_OK;
	static qapi_I2CM_Descriptor_t desc[2];	/* 必须静态 */
	qapi_I2CM_Config_t config;				/* 绝对不能静态 */
	static unsigned int transferred1 = 0, transferred2 = 0;	/* 必须静态 */
	static uint8 w_reg[2];					/* 必须静态 */
...
//	desc[0].length	   = 1;
	...
	res = qapi_I2CM_Transfer (i2c_handle, &config, desc, 2, i2c_callback, &CB_Parameter, 100);
...
qapi_Status_t i2c_read_reg_block(uint8 device_Address, uint16 reg, uint8 *buff, I2C_REG_TYPE_t reg_type)
{
	...
	int cnt;
	res = i2c_read_reg(device_Address, reg, buff, reg_type);
