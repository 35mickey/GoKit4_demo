...
static uint8_t gps_reset_cnt = 0;
...
static void location_tracking_callback(qapi_Location_t loc)
{
	...
	/* If not tracking for more than one second, location.flags will be reset. Reset cnt to prevent it */
	gps_reset_cnt = 0;
...
/**
* @brief Timer callback function, in the function to reset gps flag if no tracking data
* @param none
* @return none
*/
void gpsTimerFunc(void)
{
	gps_reset_cnt++;
	/* If no tracking for more than 3 senconds */
	if(gps_reset_cnt > 6)
		location.flags &= ~((uint8)QAPI_LOCATION_HAS_LAT_LONG_BIT);	/* This bit is for GPS lock judgement. */
}
